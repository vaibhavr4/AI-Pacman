    ...
    for housePos in house_locations:
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for row in obsCPT:
                ...
                color = row[obsVar]
                adjPos = getAdjPos(gameState, obsPos)
                prob = 0

                if ghostHouseVal != adjPos   and foodHouseVal != adjPos:
                    if color == RED_OBS_VAL or color == BLUE_OBS_VAL:
                        prob = 0
                    elif color == NO_OBS_VAL:
                        prob = 1
                elif ghostHouseVal == foodHouseVal or foodHouseVal == adjPos:
...
def getAdjPos(gameState, obsPos):
    dist = float("inf")
    adjacentHouse = None
...
class VPIAgent(BayesAgent):
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for (prob, evidence0) in outcomes:
