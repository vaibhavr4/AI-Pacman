...
def joinFactors(factors):
    ...
    for f in factors:
        temp_uncond = f.unconditionedVariables()
        temp_cond = f.conditionedVariables()
        uncond.extend(temp_uncond)
        for conditioned_var in temp_cond:
            if conditioned_var not in cond:
                cond.append(conditioned_var)
    cond = [var for var in cond if var not in uncond]

    newFactor = Factor(uncond, cond, variableDomainsDict)
    assignments = newFactor.getAllPossibleAssignmentDicts()
    for assign in assignments:
        prob = 1
        for factor in factors:
            prob *= factor.getProbability(assign)
        newFactor.setProbability(assign, prob)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        uncond = [var for var in uncond if var != eliminationVariable]
        ...
        for assignment in newFactor.getAllPossibleAssignmentDicts():
            prob = 0
            for rmvVal in domain:
                old_assignment = assignment.copy()
                old_assignment[eliminationVariable] = rmvVal
...
def normalize(factor):
    ...
    for f in factor.unconditionedVariables():
        if len(variableDomainsDict[f]) ==1:
            condVar.add(f)
            uncondVar.remove(f)
    newFactor = Factor(uncondVar, condVar, variableDomainsDict)
    for assignment in newFactor.getAllPossibleAssignmentDicts():
